...
| --- | --- | --- |
| **Private** **action** **addressing** | **Optional** **extension** **`relay.ext.private_action.v1`**: **tag** (SHA-256 of **§4.1** **canonical** **`relay.action.tag.v1`**) + **`encrypted_payload` **;** **plaintext** **`target`** **omitted** **;** **§13.4** **commitment** on **decrypted** **payload** **per** **§13.5** **. | **§13.5** |
...
| **Materialized feed outputs; cross-deployment feed audit** | v1.4: **only** a **SHOULD** (**§11.1** / **§17.11**) to **pair** **exports** with **definition** `object_id` + **`version`**. There is **no** **MUST-level** **universal** **metadata** **format** for **materialized** **outputs**—**feed** **audit** or **diff** **tools** **spanning** **deployments** **lack** a **guaranteed** **common** **envelope** without **OOB** **agreement** or **a** **v1.5+** **normative** (or **profile-gated**) **format** (e.g. **required** **fields** **pairing** **results** with `{ definition object_id, version, as_of }`). **MUST** **would** **overconstrain** **caching** **/ sharing**; **v1.4** **keeps** **SHOULD** (see **§11.1** rationale). |
...
* **`data.action` fragmentation (largest practical interop gap):** The **framing** in **§13.4**, **§23.2**, and the **deferred** table row **`data.action` string registry / semantic equivalence** is **deliberate**—**verifiers** **MUST** **treat** **mismatched** **strings** as **different** **actions** in **conformance** and **security** **analysis** **even** if **product** **marketing** conflates them **(§23.2).** **In** **practice** the **action** **event** **model** is **most** **reliable** **inside** **ecosystems** **that** **agree** on **`data.action`** **values** **out-of-band**; **full** **cross-product** **semantic** **equivalence** for **arbitrary** **strings** **is** **not** **solved** in v1.4 and **rests** on **v1.5+** **registry** (or **profile**) **work** in the **deferred** table.
...
* **SHOULD-level export tagging and cross-deployment feed-audit / timeline-diff tooling:** v1.4’s **SHOULD** **(not** **MUST**)** to **tag** **exports** with **`object_id` +** **`version`** **(§11.1** / **§17.11;** **deferred** **row** *Materialized feed outputs; cross-deployment feed audit*) **leaves** **no** **guaranteed** **common** **metadata** **envelope** for **comparing** **or** **auditing** **feeds** **across** **unrelated** **deployments** **. **Such** **tooling** **therefore** **needs** **out-of-band** **agreement** on **envelope** **fields** **(or** a **later** **v1.5+** **normative** **format**)** **until** **profile** or **core** **work** **closes** the **gap**; **v1.4** **states** that **tradeoff** **explicitly** **(flexible** **caching** / **sharing** **vs** **universal** **tags**)** **.**
...
* **`data.action` interop (normative: §10.2, §13.4, §22; deferred table).** *Commentary only:* two honest deployments can use different strings for the same human intent; verifiers only see signed bytes. **This does not** relax **§13.4** (parse, sign, and hash) for a **given** `data.action` value.
...
* **Feed definition as mutable state; “feed audit” tooling (normative: §11.1, §16 `version`, §17.11; SHOULD in §11.1; deferred table).** *Commentary only:* the gap between **SHOULD**-level tagging of **exports** with `object_id` + `version` and a **hypothetical** **MUST**-level **universal** **envelope** matters for **cross-deployment** “feed audit” or diff tools—**no** **guaranteed** **common** **metadata** **format** in v1.4; see **deferred** row for v1.5+ candidates.
...
* **Recompute cost (normative: §17.11, including “Scalability and what the MUST does not require”).** *Commentary only:* the **MUST** is **possession** of a **correct recompute** **procedure** when **auditing**; not **per-frame** full recompute. **§17.11** **states** that **caching** / **spot-checks** make assurance **probabilistic** in practice.
...
* **v1.5 exception (§10.5, log events only):** a **log** event **(§10) ** with **`storage_class`:** **`log` **MAY** **use** **`content_class`:** **`ephemeral`** or **`revocable` **only** when **`expires_at` **(§10.5) **is** **present** on **that** **event** **;** **all** **§10.5** **MUST**s **MUST** **be** **satisfied** **. **(Without **`expires_at`**, the **row**s **above** **apply** **unchanged: **`ephemeral`** **/ **`revocable` **MUST** **use** **`storage_class`:** **`state` **as** **before** **. **) **
...
* **Origins and mirrors, fetch** **(§17.3, **§17.4) **: ** when **servicing** **`GET` **/actors/.../log/...` **(by** **event** **id** **in** **§17.3** **or** **range** **in** **§17.4) **, **origins** **and** **mirrors** **MUST** **not** **return** a **log** **event** **in** the **body** of **a** **response** **to** **new** **fetch** **requests** **(default** **semantics) ** if **the** **event** **has** a **present** **`expires_at` **and** the **server**’**s** **parsed** **comparison** **instant** **(UTC,** **§4.1.1.1) ** is **strictly** **after** the **parsed** **`expires_at` **instant** **(the** **event** **is** **expired** **for** **that** **response) **. **Implementations** **MUST** **compare** **parsed** **RFC** **3339** **instants,** **not** **raw** **timestamps** **strings** **as** **opaque** **sort** **keys** **. **

* **Mirrors** **(retention** **vs** **serve) **: **mirrors** **MAY** **retain** **expired** **events** **locally** **for** **audit** **;** **HTTP** **`GET` **responses** **MUST** **still** **obey** **the** **preceding** **bullet** **unless** **`include_expired=true` **(or** **documented** **alias) ** **is** **used** **per** **the** **range**-**query** **bullet** **below** **. **

* **Range** **queries** **(§17.4) **, **advisory** **query** **parameter: ** for **`GET` **/actors/.../log?...` **, **mirrors** **and** **origins** **MUST** **not** **include** **expired** **events** in **the** **default** **(no** **parameter) ** result **. **A** **request** **MAY** **include** **`include_expired=true` **(boolean) **;** when **this** **name** (or a **documented** **alias) **is **set** **and** the **client** is **permitted,** the **server** **MAY** **include** **expired** **events** in **the** **result** **(e.g. **local** **audit) **. **Support** for **`include_expired` **is** **optional** **. **(Exact** **query** **shape** is **operator**-**defined** **;** **v1.5** **recommends** the **name **`include_expired` **. **) **

* **Fast** **Relay, ** `PUB` **(§18.3) **: **a** **Fast** **Relay** **MUST** **not** **fan** **out** a **log** **envelope** **whose **`expires_at` **(if** **present) ** has **already** **passed** **(per** **the** **relay**'**s** **clock) **at** the **time** the **relay** **accepts** **or** **would** **forward** **a **`PUB` **. **(Align** **validity** **with** **§4.1.1.1** **. **) **
...
**Optional** **`action_namespace` and `action_version` (v1.4, MAY, interoperability hook):** **`data` MAY** also **include** **`action_namespace`**, a **string**, **and** **/** **or** **`action_version`**, a **string** (e.g. a **dotted** **namespace** and **a** **version** **label**). **v1.4** **does** **not** **require** or **define** a **global** **registry** of **valid** **values;** they **are** an **optional** **pairing** **hint** for **cross-** **client** **UI,** **discovery,** and **indexing,** and **MUST** **be** **UTF-8** **strings** **(NFC,** **§4.1.2) ** **when** **present** **. **The** **commitment** **( **`relay.action.commitment.v1` **,** **below) **MUST** **include** the **corresponding** **optional** **fields** **when** **present** **on** the **request** so **verifiers** **recompute** **the** **same** **hash. **Mismatches** in **`action` / `action_namespace` / `action_version` (including** **presence** **vs** **absence) **MUST** be **treated** **as** **different** **byte-** **level** **operations** (same **rule** as **for** **`data.action` **alone;** **see** **status** **deferred** **row** **on** **semantic** **equivalence) **. **

**Example (illustrative, non-exhaustive):** `{ "action": "summarize", "action_namespace": "relay.ai.basic", "action_version": "1.0", "input_refs": [ "relay:obj:…" ] }` **(with** **`input_refs` **sorted** per **this** **section) **. **
...
**Top-** **level** **`data` **(normative) **: ** the **log** **event** **`data` **MUST** be **an** **empty** **JSON** **object** **`{ }` **(or** **omitted,** **treated** **as** **`{ }` **) **. **The** **plaintext** **`data` **fields** **for** **`action.request` **( **`action`**, **`input_refs`**,** **and** any **optional** **`action_namespace` / `action_version` **per** **Appendix** **C,** **§13.4) **MUST** **appear** **only** **in** the **decrypted** **content** **of** **`encrypted_payload` **(§4.1) **. **
...
* **v1.5 (§10.5) **: **if** **`expires_at` **is **present,** **MUST** **satisfy** **§10.5 **( **`content_class` **,** **pairing,** **RFC** **3339) **; **MUST** **reject** the **append** **otherwise** **(including** **`durable_public` **+ **`expires_at` **) **. **
...
* **v1.5 (§10.5) **: **MUST** **obey** **envelope** **`expires_at` **: **MUST** **not** **return** **a** **log** **event** to **a** **“** **new** **”** **fetcher** if **it** is **expired** **as** **defined** in **§10.5. **(Exact** **HTTP** **status** / **omission** **is** **implementation**-**defined;** **stale** **caches** **MUST** **reconcile** **to** the **rule** **. **) **
...
* **v1.5 (§10.5) **: **default** **(no** **`include_expired` **) ** result **MUST** **not** **list** **expired** **events** **(§10.5) **. **`include_expired` **(or** **local** **alias) **: **per** **§10.5** **. **
...
2. **Fetch** the **constituent** **logs** / **state** per **`sources`**, according to the **reducer** **rules** in **§17.10** (and **any** **extension** **spec** for **that** `reduce` **id** if **used**), **with** an **input** **boundary** **(§17.10) ** that **MUST** be **sufficient** **to** support **reproducible** **recompute** **(bounded** **window** and/or **snapshot+** **delta,** **as** **documented) **. **
...
* **v1.5 (§10.5) **: **MUST** **not** **fan** **out** a **log** **envelope** on **`PUB` **(and **MUST** **not** **re-** **emit** an **expired** **envelope) **if **`expires_at` **(when ** present ** ) ** is **in** the **past** at **the** **relay** **(§10.5) **. **
...
| `action.request` (v1.4) | `{ "action": "<string>", "input_refs": [ "<id>", "…" ] }`; **MAY** include optional **`action_namespace`**, **`action_version`** **(UTF-8** **strings) ** | **`input_refs` MUST** be **sorted** ascending (Unicode). **§10.2.1:** **`target` MUST** = **agent** `actor_id`. **Optional** **fields** **participate** in **`relay.action.commitment.v1` **when** **present** **(§13.4) **. ** |
...
**`relay.action.commitment.v1` (recomputation only; not a free-standing wire object in v1.4):** `kind` = `relay.action.commitment.v1` — `request_event_id` = **`data.request_event_id` on the commit** — `data_action` = **`data.action` from `action.request`** — `data_action_namespace` = **copy** **`data.action_namespace` **if** **present** **(else** **omit** **key) **— `data_action_version` = **copy** **`data.action_version` **if** **present** **(else** **omit** **key) **— `input_refs` = **sorted** **`data.input_refs` from `action.request`** — `agent_params` = **`data.agent_params` from `action.commit`**. The **`commitment_hash` field** on **commit and result** = **lowercase** **64**-char **hex** of **SHA-256** of **UTF-8** of **canonical JSON** of that object per **§4.1** (the commitment object **itself** has **no** `sig` field; **§7** applies to the **enclosing** log event **as usual**).
...
| C10b | **v1.5:** If **`expires_at` **is **present** on **a** **log** **event,** **origins** **and** **Fast** **Relays** **MUST** **enforce** it **per** **§10.5;** **MUST** **not** **serve** or **fan** **out** **expired** **events** **to** **new** **requesters** **(mirrors** **and** **range** **queries: ** **§10.5,** **§17.3,** **§17.4) **. **| §10.5, §16.3, §17.3, §17.4, §18.3 |
