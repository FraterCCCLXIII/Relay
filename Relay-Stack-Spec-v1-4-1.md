...
* **Nested `kind: "feed"` / outer `reduce` (normative: §17.10, §11.1 table).** *Commentary only:* v1.4 does not enumerate every merge corner for all reducer pairs; see **§17.10** for composition rules. Registry or v1.5+ may add **(outer, inner)** pair rules.
...
| `feed` | `object_id` (string) | **Logical** `object_id` of a **`relay.feed.definition.v1`** state object (**§11**; e.g. `post:…`). **Not** a **content-addressed** or **immutability** id; **nesting** **does** **not** **pin** a **specific** **definition** **`version`**. **Verifiers** **MUST** **fetch** **that** **state** **via** **§17.5** **at** **recompute** **time** and **expand** per **§17.10** **(nested** **`reduce`**,** then** **outer** **merge**). **The** same **`object_id`**,** **higher** **`version`**,** on **edit** **⇒** a **nested** **recompute** may **legitimately** **differ** **while** the **id** **stays** **fixed** **. |
...
**Top-** **level** **`data` **(normative) **: ** the **log** **event** **`data` **MUST** be **an** **empty** **JSON** **object** **`{ }` **(or** **omitted,** **treated** **as** **`{ }` **) **. **The** **plaintext** **`data` **fields** **for** **`action.request` **( **`action`**, **`input_refs`**,** **per** **Appendix** **C) **MUST** **appear** **only** **in** the **decrypted** **content** **of** **`encrypted_payload` **(§4.1) **. **
